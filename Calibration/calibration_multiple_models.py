...
def load_arpa_data(fault_free_data, faulty_data, arpa_col='pm25'):
	...
	arpa_filtered = arpa_df[
		(arpa_df['unix_timestamp'] >= min_ts) & 
		(arpa_df['unix_timestamp'] <= max_ts)
	].copy()
...
def train_calibration_models(aligned_ff_fa_data, method='linear', knn_params=None, svr_params=None, linear_svr=False, xgb_params=None):
    print(f"Training calibration models using method='{method}'...")
    models = {}
    ...
    for gran_str, time_frames in aligned_ff_fa_data.items():
        for tf_name, days in time_frames.items():
            ...
            if not dfs:
                print(f"  > Warning: No F+A data found for {gran_str}/{tf_name}. Skipping training.")
                continue

            all_days_df = pd.concat(dfs, ignore_index=True)

            if all_days_df.empty or all_days_df.shape[0] < 2:
                print(f"  > Warning: Not enough data to train model for {gran_str}/{tf_name}.")
                continue

            X = all_days_df[[AGGREGATED_COL]]
            y = all_days_df[ARPA_COL]
            ...
            models[(gran_str, tf_name, AGGREGATED_COL)] = model
...
def save_models(models, base_dir, collection_name):
    print(f"Saving '{collection_name}' models to '{base_dir}'...")
    
    for (gran_str, tf_name, col_name), model in models.items():
        # Create a subdirectory for the collection
        collection_dir = os.path.join(base_dir, collection_name)
        model_dir = os.path.join(collection_dir, gran_str, tf_name)
        os.makedirs(model_dir, exist_ok=True)
        
        model_filename = f"{col_name}_model.pkl"
        model_path = os.path.join(model_dir, model_filename)
        
        try:
            joblib.dump(model, model_path)
        except Exception as e:
            print(f"Warning: Could not save model {model_path}. Error: {e}")
...
def apply_calibration_to_aggregate(models, aligned_data, collection_name="", timeout_sec=60.0):
    print(f"Applying models {collection_name} to aggregate data...")

    for gran_str, time_frames in aligned_data.items():
        for tf_name, days in time_frames.items():
            model = models.get((gran_str, tf_name, AGGREGATED_COL))
            if not model:
                continue
...
def main():
	...
	aligned_ff = align_data_for_training(fault_free_data, arpa_interpolated)
	aligned_f = align_data_for_training(faulty_data, arpa_interpolated)

	aligned_ff_fa = apply_filter_and_aggregation(aligned_ff, is_faulty=False)
	aligned_f_fa = apply_filter_and_aggregation(aligned_f, is_faulty=True)
	aligned_f = apply_filter_and_aggregation(aligned_f, is_faulty=False)
	
	if method == "linear_regression":
		models_ff_fa = train_calibration_models(aligned_ff_fa)
		models_f_fa = train_calibration_models(aligned_f_fa)
		models_f = train_individual_sensor_models(aligned_f)
	elif method == "svr":
		models_ff_fa = train_calibration_models(aligned_ff_fa, method='svr', svr_params={'kernel': 'rbf', 'C': 1.0, 'epsilon': 0.1})
		models_f_fa = train_calibration_models(aligned_f_fa, method='svr', svr_params={'kernel': 'rbf', 'C': 1.0, 'epsilon': 0.1})
		models_f = train_individual_sensor_models(aligned_f, method='svr', svr_params={'kernel': 'rbf', 'C': 1.0, 'epsilon': 0.1})
	elif method == "linear_svr":
		models_ff_fa = train_calibration_models(aligned_ff_fa, method='svr', linear_svr=True, svr_params={'C': 1.0, 'epsilon': 0.1, 'max_iter': 1000})
		models_f_fa = train_calibration_models(aligned_f_fa, method='svr', linear_svr=True, svr_params={'C': 1.0, 'epsilon': 0.1, 'max_iter': 1000})
		models_f = train_individual_sensor_models(aligned_f, method='svr', linear_svr=True, svr_params={'C': 1.0, 'epsilon': 0.1, 'max_iter': 1000})
	elif method == "xgboost":
		models_ff_fa = train_calibration_models(aligned_ff_fa, method='xgboost',xgb_params={'n_estimators': 200,'max_depth': 5,'learning_rate': 0.1,'random_state': 42})
		models_f_fa = train_calibration_models(aligned_f_fa, method='xgboost',xgb_params={'n_estimators': 200,'max_depth': 5,'learning_rate': 0.1,'random_state': 42})
		models_f = train_individual_sensor_models(aligned_f, method='xgboost',xgb_params={'n_estimators': 200,'max_depth': 5,'learning_rate': 0.1,'random_state': 42})
	elif method == "knn":
		models_ff_fa = train_calibration_models(aligned_ff_fa, method='knn',knn_params={'n_neighbors': 5, 'weights': 'uniform', 'algorithm': 'kd_tree'})
		models_f_fa = train_calibration_models(aligned_f_fa, method='knn',knn_params={'n_neighbors': 5, 'weights': 'uniform', 'algorithm': 'kd_tree'})
		models_f = train_individual_sensor_models(aligned_f, method='knn',knn_params={'n_neighbors': 5, 'weights': 'uniform', 'algorithm': 'kd_tree'})
	
	save_models(models_ff_fa, MODELS_DIR, "ff_fa")
	save_models(models_f_fa, MODELS_DIR, "f_fa")
	save_models(models_f, MODELS_DIR, "f")
   
	apply_calibration_to_aggregate(models_ff_fa, aligned_ff_fa, "ff_fa_collection")
	apply_calibration_to_aggregate(models_f_fa, aligned_f_fa, "f_fa_collection")
	apply_sensor_specific_calibration(models_f, aligned_f, "f_collection")
