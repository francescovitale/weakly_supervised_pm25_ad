...
CALIBRATED_OUTPUT_DIR = os.path.join(OUTPUT_DIR, "calibrated_output")
...
def apply_aggregate_model_to_individual_sensors(models, aligned_raw_data, collection_name="", timeout_sec=60.0):
    print(f"Applying aggregate models from '{collection_name}' to individual raw sensor columns...")

    for gran_str, time_frames in aligned_raw_data.items():
        for tf_name, days in time_frames.items():
            aggregate_model = models.get((gran_str, tf_name, AGGREGATED_COL))
            
            if not aggregate_model:
                continue 
            ...
            for day_str, df in days.items():
                ...
                for col in SENSOR_COLS:
                    ...
                    if not valid_rows.empty:
                        ...
                        pred_vals = safe_predict(aggregate_model, valid_rows_renamed, timeout_sec)
                        df.loc[valid_rows.index, f'cal_{col}'] = pred_vals

    print("...Individual sensor calibration (using aggregate model) complete.")
...
def main():
	...
	print("\n[Training aggregate models on FAULT-FREE data only]...")
	
	aggregate_models = {}

	if method == "linear_regression":
		aggregate_models = train_calibration_models(aligned_ff_fa)
	
	elif method == "svr":
		aggregate_models = train_calibration_models(aligned_ff_fa, method='svr', svr_params={'kernel': 'rbf', 'C': 1.0, 'epsilon': 0.1})
	
	elif method == "linear_svr":
		aggregate_models = train_calibration_models(aligned_ff_fa, method='svr', linear_svr=True, svr_params={'C': 1.0, 'epsilon': 0.1, 'max_iter': 1000})
	
	elif method == "xgboost":
		aggregate_models = train_calibration_models(aligned_ff_fa, method='xgboost',xgb_params={'n_estimators': 200,'max_depth': 5,'learning_rate': 0.1,'random_state': 42})
	
	elif method == "knn":
		aggregate_models = train_calibration_models(aligned_ff_fa, method='knn',knn_params={'n_neighbors': 5, 'weights': 'uniform', 'algorithm': 'kd_tree'})
	
	save_models(aggregate_models, MODELS_DIR, "aggregate_model")
   
	print("\n[Applying AGGREGATE models (trained on ff) to fault-free and faulty-fa data...]")
	apply_calibration_to_aggregate(aggregate_models, aligned_ff_fa, "aggregate_model")
	apply_calibration_to_aggregate(aggregate_models, aligned_f_fa, "aggregate_model")
	
	print("\n[Applying AGGREGATE models (trained on ff) to raw faulty sensor data...]")
	apply_aggregate_model_to_individual_sensors(aggregate_models, aligned_f, "aggregate_model")
